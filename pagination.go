...
func GetStudents(qstr string) []Student {
	...
	stds := []Student{}
	for std_row.Next() {
		err = std_row.Scan(&std.id, &std.Sid, &std.Sname, &std.Sphone, &std.Slvl, &std.Sen_date, &std.Re_date, &std.Stype, &std.Entime, &std.Stime, &std.Pic_path)
		CheckErr(err)
	...
	if count > 0 {
		for _, i := range stds {
			...
			<tr>
				...
				<td>` + i.Entime + `</td>
				<td>` + i.Stime + `</td>
				<td> <button class="popup btn btn--radius-2 btn--red m-r-55" data-id=` + strconv.Itoa(i.id) + ` data-name= " ` + i.Sname + `"  data-phone= " ` + i.Sphone + `"  data-toggle="modal" data-target="#modalForm"> <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit </button> </td>
	...
	if count > 0 {
		for _, i := range stds {
			...
			<tr>
				...
				<td>` + i.Sname + `</td>
				<td>` + i.Slvl + `</td>
				<td> <button class="popup btn btn--radius-2 btn--red m-r-55" data-id="` + strconv.Itoa(i.id) + `" data-toggle="modal" data-target="#modalForm"><i class="fa fa-user-plus" aria-hidden="true"></i>  Enroll </button> </td>
	...
	if count > 0 {
		for _, i := range stds {
			...
			<tr>
				...
				<td>` + i.Sname + `</td>
				<td>` + i.Slvl + `</td>
				<td> <button class="popup btn btn--radius-2 btn--red m-r-55" data-id="` + strconv.Itoa(i.id) + `" data-toggle="modal" data-target="#modalForm"> Statement </button> </td>
...
/*
func stdsImgPagetHandler(w http.ResponseWriter, r *http.Request){
	templates.ExecuteTemplate(w, "stds_img.html", nil)
}
// Generate Pagination for Display Students For Admin
func getstdImgPage(w http.ResponseWriter, r *http.Request){
	...
	if count > 0 {
		for _,i := range stds{
			...
			<tr>
				...
				<td>` + strconv.Itoa(i.id) +`</td>
				<td> <img src="../static/std_imgs/m7md.png" style="width: 50px;height: auto"> </td>
				<td>` + i.Sname +`</td>
				<td>` + i.Slvl +`</td>
				<td> <button class="popup btn btn--radius-2 btn--red m-r-55" data-id=`+ strconv.Itoa(i.id) +` data-name= " ` + i.Sname +`"  data-phone= " `+ i.Sphone +`"  data-toggle="modal" data-target="#modalForm"> <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Add/Edit Image </button> </td>
